...
import com.swiftsave.app.data.AppDatabase
import com.swiftsave.app.data.DownloadEntity
import com.swiftsave.app.data.DownloadStatus
import com.swiftsave.app.data.UserPreferencesRepository
import com.swiftsave.app.data.buildAppDatabase
...
class SwiftSaveApplication : Application() {
    ...
    private fun warmUpYoutubeDlInBackground() {
        applicationScope.launch {
            runCatching { YoutubeDlInitializer.ensureInitialized(this@SwiftSaveApplication) }
                .onFailure { e ->
    ...
    fun enqueueDownloadFromSharedLink(url: String) {
        ...
        applicationScope.launch {
            try {
                val quality = userPreferences.defaultQuality.first()
                ...
                DownloadForegroundService.startDownload(this@SwiftSaveApplication, jobId, safe, quality)
            } catch (e: Exception) {
    ...
    companion object {
        const val CHANNEL_ID = "swift_save_downloads"
        private const val TAG = "SwiftSave"
    }
}
