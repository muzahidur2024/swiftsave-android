...
/**
 * Parses yt-dlp [download] lines and reports progress (from youtubedl-android [StreamProcessExtractor]).
 */
internal class YtdlpProgressStreamExtractor(
    ...
    private fun getProgress(line: String): Float {
        ...
        if (matcher.find()) {
            return matcher.group(GROUP_PERCENT)?.toFloatOrNull()?.also { progress = it } ?: progress
        } else {
            val mAria2c = pAria2c.matcher(line)
            if (mAria2c.find()) {
                return mAria2c.group(1)?.toFloatOrNull()?.also { progress = it } ?: progress
            }
        }
        return progress
    }
    ...
    private fun convertToSeconds(minutes: String?, seconds: String?): Int {
        val parsedSeconds = seconds?.toIntOrNull() ?: return 0
        val parsedMinutes = minutes?.toIntOrNull() ?: return parsedSeconds
        return parsedMinutes * 60 + parsedSeconds
    }

    companion object {
        private val TAG = YtdlpProgressStreamExtractor::class.java.simpleName
