...
import com.swiftsave.app.MainActivity
...
import com.swiftsave.app.util.SavedMediaPublisher
import com.swiftsave.app.util.YoutubeDlInitializer
import com.swiftsave.app.util.YtDlpFormats
...
class DownloadForegroundService : Service() {
    ...
    private fun cancelDownload(jobId: String) {
        ...
        runCatching { YoutubeDL.getInstance().destroyProcessById(jobId) }
        scope.launch {
            (applicationContext as SwiftSaveApplication).database.downloadDao().updateStatus(
                jobId,
    ...
    private suspend fun runDownload(jobId: String, url: String, quality: String) {
        val app = applicationContext as SwiftSaveApplication
        ...
        try {
            ...
            val outDir = File(getExternalFilesDir(Environment.DIRECTORY_MOVIES), "SwiftSave").apply { mkdirs() }
            clearStaleOutputs(outDir, jobId)
            val outputTemplate = File(outDir, jobId).absolutePath + ".%(ext)s"
            val request = YoutubeDLRequest(url)
            request.addOption("-o", outputTemplate)
            YtDlpFormats.applyPerformanceOptions(request)
            YtDlpFormats.applyQuality(request, quality)
            val response =
            ...
            val msg =
                if (wasCancelled) {
                    ...
                    getString(R.string.download_error_safe)
                }
            dao.updateStatus(
                jobId,
                status,
    ...
    private fun resolveOutputFile(dir: File, jobId: String): File? {
        ...
        return dir.listFiles()?.firstOrNull { it.isFile && it.name.startsWith(jobId) }
    }
    ...
    private fun buildProgressNotification(
        ...
        val b = NotificationCompat.Builder(this, SwiftSaveApplication.CHANNEL_ID)
    ...
    companion object {
        ...
        const val ACTION_START = "com.swiftsave.app.download.START"
        const val ACTION_CANCEL = "com.swiftsave.app.download.CANCEL"
